...
import org.apache.hive.ui.internal.HiveConnectionWrapper;
...
//    }
    ...
    private Response attemptHiveConnection(String pass) {
        ConnectionConfig connectionConfig = ConnectionFactory.create();
        HiveConnectionWrapper hiveConnectionWrapper = new HiveConnectionWrapper(connectionConfig.getJdbcUrl(), connectionConfig.getUsername(), pass,new AuthParams());
        try {
    ...
    public Response setupPassword(AuthRequest request) {
        try {
            ...
            ConnectionSystem instance = ConnectionSystem.getInstance();
            instance.persistCredentials(HiveUIContext.getUsername(),request.password);
