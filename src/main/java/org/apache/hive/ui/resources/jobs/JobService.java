...
package org.apache.hive.ui.resources.jobs;
...
import org.apache.hive.ui.backgroundjobs.BackgroundJobController;
import org.apache.hive.ui.backgroundjobs.BackgroundJobException;
import org.apache.hive.ui.client.AsyncJobRunner;
import org.apache.hive.ui.client.AsyncJobRunnerImpl;
import org.apache.hive.ui.client.ColumnDescription;
import org.apache.hive.ui.client.HiveClientException;
import org.apache.hive.ui.client.NonPersistentCursor;
...
public class JobService extends BaseService {
  ...
  protected synchronized JobResourceManager getResourceManager() {
    if (resourceManager == null) {
      ...
      resourceManager = new JobResourceManager(connectionsFactory);
    }
  ...
  protected Aggregator getAggregator() {
    if (aggregator == null) {
      ...
      ActorRef operationController = ConnectionSystem.getInstance().getOperationController();
  ...
  public Response getOne(@PathParam("jobId") String jobId) {
    try {
      ...
      if(job.getStatus().equals(Job.JOB_STATE_ERROR) || job.getStatus().equals(Job.JOB_STATE_CANCELED)){
        ...
        Optional<Failure> error = asyncJobRunner.getError(jobId, HiveUIContext.getUsername());
  ...
  public Response getResultsCSV(@PathParam("jobId") String jobId,
    ...
    try {
      ...
      final String username = HiveUIContext.getUsername();
  ...
  public Response getResultsToHDFS(@PathParam("jobId") String jobId,
    ...
    try {
      ...
      if (commence != null && commence.equals("true")) {
        ...
        BackgroundJobController.getInstance().startJob(String.valueOf(backgroundJobId), new Runnable() {
          @Override
          public void run() {
      ...
      if (stop != null && stop.equals("true")) {
        BackgroundJobController.getInstance().interrupt(backgroundJobId);
      }

      JSONObject object = new JSONObject();
      object.put("stopped", BackgroundJobController.getInstance().isInterrupted(backgroundJobId));
      object.put("jobId", jobController.getJob().getId());
      object.put("backgroundJobId", backgroundJobId);
      object.put("operationType", "CSV2HDFS");
      object.put("status", BackgroundJobController.getInstance().state(backgroundJobId).toString());
  ...
  @Produces(MediaType.APPLICATION_JSON)
  public Response fetchJobStatus(@PathParam("jobId") String jobId) throws  HiveClientException, NoOperationStatusSetException {
    JobController jobController = getResourceManager().readController(jobId);
    Job job = jobController.getJob();
  ...
  public Response getResults(@PathParam("jobId") final String jobId,
    ...
    try {
      ...
      return ResultsPaginationController.getResultAsResponse(jobId, fromBeginning, count, searchId, format, requestedColumns);
  ...
  public Response keepAliveResults(@PathParam("jobId") String jobId,
                             ...
                             @QueryParam("count") Integer count) {
    try {
      if (!ResultsPaginationController.getInstance().keepAlive(jobId, ResultsPaginationController.DEFAULT_SEARCH_ID)) {
  ...
  public Response getList(@QueryParam("startTime") long startTime, @QueryParam("endTime") long endTime) {
    try {
      ...
      List<Job> allJobs = getAggregator().readAllForUserByTime("admin",startTime, endTime);
      for(Job job : allJobs) {
