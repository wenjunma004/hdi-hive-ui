...
import org.apache.hive.ui.resources.jobs.atsJobs.HiveQueryId;
import org.apache.hive.ui.resources.jobs.atsJobs.IATSParser;
import org.apache.hive.ui.resources.jobs.atsJobs.TezDagId;
...
public class Aggregator {
  ...
  private JobResourceManager viewJobResourceManager;
  private final ActorRef operationController;

  public Aggregator(JobResourceManager jobResourceManager,
  ...
  private List<Job> readJobsFromDbByJobId(List<String> jobsIds) {
    ...
    for (final String jid : jobsIds) {
      ...
      if(job == null){
        LOG.error("Error while finding job with id : {}", jid);
      }else{
        jobs.add(job);
      }
      }
   ...
   * @return: the job
   */
  private Job getJobFromDbByJobId(final String jobId)  {
    if (null == jobId)
      return null;
...
//      return jobs.get(0);
    ...
    Job job = viewJobResourceManager.read(jobId);
    if(job != null){
      return job;
    }
    return null;
//    throw new ItemNotFound(String.format("Job with id %s not found.", jobId));
  ...
  private List<Job> readDBOnlyJobs(String username, List<HiveQueryId> queries, Long startTime, Long endTime) {
    ...
    List<Job> jobs = viewJobResourceManager.readAll();
    for (Job job : jobs) {
   ...
   * @return
   */
  private JobImpl mergeHiveAtsTez(HiveQueryId atsHiveQuery, TezDagId atsTezDag, Job viewJob)  {
    saveJobInfoIfNeeded(atsHiveQuery, atsTezDag, viewJob);
    return mergeAtsJobWithViewJob(atsHiveQuery, atsTezDag, viewJob);
  }

  public Job readATSJob(Job viewJob)  {
  ...
  protected void saveJobInfoIfNeeded(HiveQueryId hiveQueryId, TezDagId tezDagId, Job viewJob, boolean useActorSystem)  {
    ...
    if(updateDb) {
      if (useActorSystem) {
        ...
        viewJobResourceManager.update(viewJob);
      }
    }
  }
  ...
  protected Job getJobByOperationId(final String opId) {

    List<Job> jobs = viewJobResourceManager.findJobsByGuiId(opId);
//    List<Job> jobs = viewJobResourceManager.readAll(new FilteringStrategy() {
//      @Override
//      public boolean isConform(Indexed item) {
